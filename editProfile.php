<?php
  ...
  if (isset($_POST['screenName'])) {
    ...
    if (strlen($screenName) < 5 OR strlen($screenName) > 20) {
      $error = 'Username must be in between 5-20 chars';
    }
    if (strlen($profileBio) > 150) {
      $error = 'Profile Bio is too long';
    }
    if (strlen($country) > 24) {
      $error = 'Country name is not valid';
    }
    if (strlen($website) > 50) {
      $error = 'Website name not valid';
    }
    if (!isset($error)) {
  ...
  if (isset($_FILES['profileImage'])) {
    if (isset($_FILES['profileImage']['name'][0])) {
  ...
  if (isset($_FILES['profileCover'])) {
    if (isset($_FILES['profileCover']['name'][0])) {
