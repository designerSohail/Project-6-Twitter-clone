    ...
    if (isset($_POST['next'])) {
      ...
      if (!empty($username)) {
        if (strlen($username) > 20 OR strlen($username) < 5) {
          $error = 'Please Enter a username between 5-20 chars';
        }
        if ($getFromU->checkExistingUsername($username) === true) {
          $error = 'Username with ' . $username . 'already Exist Please choose another';
        }
        if (!isset($error)) {
          $getFromU->update('users', $user_id, array('username' => $username));
          header('Location: signup-full.php?step=2');
        }
      } else {
        $error = 'Pleaes Enter a valid username';
      }
    }
  }
  ?>
